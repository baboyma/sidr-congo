...
## VIZ --------------------------------------------------
    ...
    si_save("FY20_IndexTesting_Adults", path = dir_graphics)
    ...
    si_save("FY20_IndexTesting_adults_by_agency", path = dir_graphics)
    ...
    si_save("FY20_IndexTesting_adults_USAID_SNU1", path = dir_graphics)
    ...
    ggplot() +
        ...
        geom_point(data = df_psnu_idx_im2,
                   aes(x = mech_name, y = measurement * 150),
                   shape = 21, size = 10, color = si_lorange, fill = "white") +
        geom_text(data = df_psnu_idx_im2,
                  aes(mech_name, measurement * 150, label = paste0(round(measurement, 1), "%"), group = standardizeddisaggregate),
                  position = position_dodge(width = 1), hjust = .5, vjust = .5, color = grey80k, size = 3, show.legend = F) +
        scale_fill_manual(values = c("#7fc97f", "#beaed4", "#fdc086")) +
        scale_y_continuous(
            name = "Cases | Contacts | New POS",
            sec.axis = sec_axis(trans = ~./150, name = "Index Yield (%)")
        ) +
        labs(
            title = "DRC - Index Cases, Contacts & Testing Results",
    ...
    si_save("FY20_IndexTesting_adults_USAID_IM", path = dir_graphics)
    ...
    ggplot() +
        ...
        geom_point(data = df_psnu_idx_imq2,
                   aes(x = reporting_period, y = measurement * 150),
                   shape = 21, size = 10, color = si_lorange, fill = "white") +
        geom_text(data = df_psnu_idx_imq2,
                  aes(reporting_period, measurement * 150, label = paste0(round(measurement, 1), "%"), group = standardizeddisaggregate),
                  position = position_dodge(width = 1), hjust = .5, vjust = .5, color = grey80k, size = 3, show.legend = F) +
        scale_fill_manual(values = c("#7fc97f", "#beaed4", "#fdc086")) +
        scale_y_continuous(
            name = "Cases | Contacts | New POS",
    ...
    si_save("FY20_IndexTesting_adults_USAID_IM_Qtr", path = dir_graphics)
    ...
    df_ou_idx_cases %>%
        ...
        summarise(measurement = sum(measurement, na.rm = TRUE)) %>%
        ungroup() %>%
        ggplot(aes(standardizeddisaggregate, measurement)) +
        geom_col(fill = si_lorange, position = "dodge", show.legend = F) +
        geom_label(aes(label = comma(measurement)), fill = "white", color = "grey50", show.legend = F) +
        scale_y_continuous(position = "right", labels = comma) +
        coord_flip() +
        labs(
            title = "DRC - Index Testing Cases & Contacts",
            subtitle = "FY20 cumulative and grouped at OU level.\nContacts represent 1.06% of Index Cases.",
            caption = graph_note,
            x = "",
            y = ""
        ) +
    ...
    si_save("FY20_Index_CasesContacts", path = dir_graphics)

    ## OU Cases Aggregates by Agency
    df_ou_idx_cases %>%
        group_by(fundingagency, standardizeddisaggregate) %>%
        summarise(measurement = sum(measurement, na.rm = TRUE)) %>%
        ungroup() %>%
        ggplot(aes(standardizeddisaggregate, measurement)) +
        geom_col(aes(fill = fundingagency), position = "dodge", show.legend = F) +
        geom_label(aes(label = comma(measurement)), fill = "white", color = "grey50", show.legend = F) +
        scale_y_continuous(position = "right", labels = comma) +
        scale_fill_manual(values = c(si_blue, si_lblue, grey50k)) +
        coord_flip() +
        facet_wrap(~fundingagency) +
        labs(
            title = "DRC - Index Testing Cases & Contacts",
            subtitle = "FY20 cumualtive and disaggregated by agency",
            caption = graph_note,
            x = "",
            y = ""
        ) +
    ...
    si_save("FY20_Index_CasesContacts_by_Agency", path = dir_graphics)
    ...
    df_ou_idx_cases %>%
        ...
        scale_y_continuous(position = "right", labels = comma) +
        coord_flip() +
        facet_wrap(~reporting_period) +
        labs(
            title = "DRC - Index Testing Cases & Contacts",
            subtitle = "FY20 results by reporting periods",
            caption = graph_note,
            x = "",
            y = ""
        ) +
    ...
    si_save("FY20_Index_CasesContacts_by_period", path = dir_graphics)


    ## OU Cases by Agency and Period
    df_ou_idx_cases %>%
        mutate(fundingagency = factor(fundingagency, levels = c("USAID", "CDC", "DOD"))) %>%
        ggplot(aes(standardizeddisaggregate, measurement, fill = fundingagency)) +
        geom_col(position = "dodge", show.legend = F) +
        geom_text(aes(x = standardizeddisaggregate, y =  200, label = comma(measurement)),
                  color = grey90k, size = 2.5, fontface = "bold",
                  hjust = 0, show.legend = F) +
        scale_fill_manual(values = c(si_blue, si_lblue, grey50k)) +
        coord_flip() +
        facet_wrap(fundingagency ~ reporting_period) +
        labs(
            title = "DRC - Index Testing Cases & Contacts",
            subtitle = "FY20 results by periods and funding agency",
            caption = graph_note,
            x = "",
            y = ""
        ) +
    ...
    si_save("FY20_Index_CasesContacts_by_agency_and_period", path = dir_graphics)
    ...
    df_ou_idx_cases %>%
        ...
        group_by(standardizeddisaggregate, fundingagency) %>%
        summarise(measurement = sum(measurement, na.rm = TRUE)) %>%
        ungroup() %>%
        ggplot(aes(reorder(standardizeddisaggregate, measurement), measurement, group = fundingagency)) +
        geom_col(aes(fill = standardizeddisaggregate), position = "dodge", show.legend = F) +
        geom_label(aes(label = measurement, fill = standardizeddisaggregate), color = "white", show.legend = F) +
        scale_y_continuous(position = "right") +
        facet_grid(fundingagency ~ ., switch = "y") +
        coord_flip() +
        labs(
            title = "Distribution of Index Testing Cases & Contacts",
            subtitle = "Numbers are cumulative and grouped by funding agency",
            caption = graph_note,
            x = "",
            y = ""
        ) +
    ...
    df_mer_idx_tst %>%
        filter(trendscoarse == "15+") %>%
        group_by(standardizeddisaggregate, fundingagency, metrics) %>%
        summarise(measurement = sum(measurement, na.rm = TRUE)) %>%
        ungroup() %>%
        filter(metrics != "qtr4") %>%
        mutate(metrics = toupper(metrics)) %>%
        ggplot(aes(reorder(standardizeddisaggregate, measurement), measurement)) +
        geom_col(aes(fill = standardizeddisaggregate), position = "dodge", show.legend = FALSE) +
        geom_label(aes(label = measurement, fill = standardizeddisaggregate), color = "white", show.legend = F) +
        scale_y_continuous(position = "right") +
        #coord_flip() +
        facet_grid(fundingagency ~ metrics, switch = "both") +
        labs(
            title = "Distribution of Index Testing Cases & Contacts",
            subtitle = "Results are summed at OU level and grouped by Agency and Reporting period",
            caption = graph_note,
            x = "",
            y = ""
        ) +
    ...
    df_mer_idx_tst %>%
        filter(fundingagency == "USAID", trendscoarse == "15+") %>%
        group_by(standardizeddisaggregate, primepartner, metrics) %>%
        summarise(measurement = sum(measurement, na.rm = TRUE)) %>%
        ungroup() %>%
        filter(metrics != "qtr4") %>%
        mutate(metrics = str_remove(toupper(metrics), "TR")) %>%
        ggplot(aes(reorder(standardizeddisaggregate, measurement), measurement)) +
        geom_col(aes(fill = standardizeddisaggregate), position = "dodge", show.legend = FALSE) +
        geom_label(
            aes(label = scales::comma(measurement, accuracy = 1), fill = standardizeddisaggregate),
            angle = 90, hjust = .5, color = "white", size = 2.5, show.legend = F) +
        scale_y_continuous(position = "right", labels = scales::comma) +
        #coord_flip() +
        facet_grid(primepartner ~ metrics, switch = "both") +
        labs(
            title = "Distribution of Index Testing Cases & Contacts (USAID)",
            subtitle = "Results are summarised at OU level and grouped by Parter and Reporting period",
            caption = graph_note,
            x = "",
            y = ""
        ) +
    ...
    df_mer_idx_tst %>%
        filter(fundingagency == "USAID", trendscoarse == "15+") %>%
        group_by(standardizeddisaggregate, snu1, metrics) %>%
        summarise(measurement = sum(measurement, na.rm = TRUE)) %>%
        ungroup() %>%
        filter(metrics != "qtr4") %>%
        mutate(metrics = str_remove(toupper(metrics), "TR")) %>%
        ...
        geom_label(
            ...
            angle = 90, hjust = .5, color = "white", size = 2.5, show.legend = F) +
        scale_y_continuous(position = "right") +
        facet_grid(snu1 ~ metrics, switch = "both") +
        labs(
            title = "Distribution of Index Testing Cases & Contacts (USAID)",
            subtitle = "Results are summarised at OU level and grouped by SNU1 and Reporting period",
            caption = graph_note,
            x = "",
            y = ""
        ) +
    ...
    df_mer_idx_tst %>%
        filter(fundingagency == "USAID", trendscoarse == "15+") %>%
        group_by(standardizeddisaggregate, psnu, metrics) %>%
        summarise(measurement = sum(measurement, na.rm = TRUE)) %>%
        ungroup() %>%
        filter(metrics != "qtr4") %>%
        mutate(metrics = str_remove(toupper(metrics), "TR")) %>%
        ...
        geom_label(
            ...
            angle = 90, hjust = .5, color = "white", size = 2.5, show.legend = F) +
        scale_y_continuous(position = "right") +
        facet_grid(psnu ~ metrics, switch = "both") +
        labs(
            title = "Distribution of Index Testing Cases & Contacts (USAID)",
            subtitle = "Results are summarised at OU level and grouped by SNU1 and Reporting period",
            caption = graph_note,
    ...
    df_mer_idx_tst %>%
        ggplot(aes(fundingagency, measurement)) +
        geom_col(aes(fill = standardizeddisaggregate), position = "dodge") +
        facet_grid(trendscoarse ~ sex, switch = "y") +
        coord_flip() +
        labs(
            title = "Distribution of Index Testing Cases & Contacts",
            subtitle = "Numbers are cumulative and grouped by Agency, Sex and Age",
            caption = paste0("USAID/OHA/SIEI - HQ Q3 Data Deep Dives, ", Sys.Date()),
            x = "",
            y = ""
        ) +
        si_style_xgrid() +
    ...
    df_mer_idx_tst %>%
        ...
        ggplot(aes(fundingagency, cumulative)) +
        geom_col(aes(fill = standardizeddisaggregate), position = "dodge") +
        facet_grid(trendscoarse ~ sex) +
        coord_flip() +
        si_style() +
        theme(legend.title = element_blank())
    ...
    ## Index Contacts Testing Results
    df_mer_idx_rst %>%
        ggplot(aes(fundingagency, measurement)) +
        geom_col(aes(fill = standardizeddisaggregate), position = "fill") +
        facet_grid(trendscoarse ~ sex) +
        coord_flip() +
        labs(title = "Index Testing Results by Agency, Age and Sex") +
        si_style_xgrid() +
        theme(legend.title = element_blank())

    df_mer_idx_rst %>%
        filter(fundingagency == "USAID") %>%
        ggplot(aes(snu1, measurement)) +
        geom_col(aes(fill = standardizeddisaggregate), position = "fill") +
        facet_grid(trendscoarse ~ sex) +
        coord_flip() +
        labs(title = "Index Testing Results (USAID) by SNU1, Age and Sex") +
        si_style_xgrid() +
        theme(legend.title = element_blank())
